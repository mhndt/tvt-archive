...
class ReleaseStaticTests(unittest.TestCase):
    ...
    def test_panel_module_url_is_content_versioned(self) -> None:
        ...
        self.assertIn("panel_digest = sha256(panel_path.read_bytes()).hexdigest()[:12]", init_text)
        self.assertIn('panel_module_url = f"{PANEL_MODULE_PATH}?v={panel_digest}"', init_text)
        self.assertIn("module_url=panel_module_url", init_text)
        self.assertNotIn("PANEL_MODULE_URL", const_text)
        self.assertNotIn("?v=0.8.2", const_text)
